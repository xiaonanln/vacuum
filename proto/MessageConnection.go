...
func (mc MessageConnection) SendMsg(mt MsgType_t, msg interface{}) error {
	...
	//defer msgbufpool.PutMsgBuf(msgbuf) // put msgbuf back
	...
	binary.LittleEndian.PutUint32((msgbuf)[:MESSAGE_SIZE_FIELD_SIZE], pktSize)
	log.Printf("Send message: size=%v, type=%v: %v", pktSize, mt, msg)
	return mc.SendAll((msgbuf)[:pktSize])
}

func (mc MessageConnection) RecvMsg() error {
	var _sizeBuf [4]byte
	pktSizeBuf := _sizeBuf[:]
	...
	var pktSize uint32 = binary.LittleEndian.Uint32(pktSizeBuf)
